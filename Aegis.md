...
`specs/INDEX.md` 追踪所有需求的状态。同时只有一个需求处于 `🔨 implementing`，L1 可插队不阻塞 L2/L3。
...
│   ├── install.ps1                      ← 一键安装脚本（从 GitHub 拉取）
│   ├── install-aegis.ps1                ← 离线备用安装脚本
│   ├── .gitignore
│   │
│   ├── rules/
│   │   ├── global.md                    ← 全局通用准则（SOLID + 安全红线 + 代码风格）
...
| 审视维度 | 具体问题 |
|----------|----------|
| 目录结构 | `specs/` 下是否出现不该放在一起的文件？命名是否一致？ |
