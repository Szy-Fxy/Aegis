...
3. 项目根目录 README.md / Aegis_Protocol.md             → 项目级入口规则
```
...
```
需求确认 → AI 直接修改代码 → git diff review
```

**规则**：
- 不生成 Markdown 文档
- 修改完成后展示 git diff 供用户确认
- 可选记录 DevLog（学到新东西或踩了坑时写）
...
| "一个文件而已，不需要diff" | 不。任何修改必须展示 diff。 |
...
```
阶段 [1/3] 方案设计 → 阶段 [2/3] 实现 → 阶段 [3/3] 验证 + DevLog
```

### 阶段 L2-1：方案设计

输出 `Aegis_Specs/L2/{feature-name}/design.md`，模板见 `templates/spec-L2.md`。

**规则**：合并文档（背景+目标+非目标+方案+接口+验收场景），AI 出方案后必须等待用户确认才能进入 L2-2。

### 阶段 L2-2：实现

AI 按 design.md 写代码，完成后运行 lint / typecheck / build 验证。

### 阶段 L2-3：验证 + DevLog

输出验证报告和开发日志：
- `Aegis_Specs/L2/{feature-name}/verify.md`，模板见 `templates/verify.md`
- `Aegis/rules/DevLogs/{项目名}_P{阶段编号}_{功能描述}_{YYYY-MM-DD}.md`

### 反借口表

| AI 可能的借口 | 反驳 |
|---|---|
| "L2 不需要 spec，直接写代码就行" | 不。L2 的 design.md 就是轻量 spec。没有它就没有验收标准。 |
| "方案很明显，不需要等确认" | 不。用户可能看到你没看到的约束。必须等确认。 |
| "测试之后补" | 不。"之后"不存在。先写验证再写代码。 |
...
| 2026-06-02 | v3.0.5 | 文件结构重构：根目录 .md 移入 docs/；版本号统一；所有引用路径更新 |