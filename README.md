...
```powershell
.\Aegis\update-aegis.ps1
```

升级脚本会保留你的 `Aegis_Specs/`、`DevLogs/`、`TempData/` 和 AI 入口文件。

或 Win+R 输入 cmd 打开 cmd 窗口：

```cmd
powershell -Command "irm https://raw.githubusercontent.com/Szy-Fxy/Aegis/main/install.ps1 | iex"
```

安装过程中会提示你选择 AI 入口（AGENTS.md / CLAUDE.md / .cursor / .trae / Copilot / Windsurf / Boot Skill），选一个最适合你平台的即可。

### 手动复制

把 `Aegis/` 文件夹复制到新项目根目录。在根目录创建 `AGENTS.md`，内容最少写一句：

```
AI 请按 Aegis/skills/dev-workflow/SKILL.md 加载规则。
```

或直接复制 `Aegis/AGENTS.md` 到项目根目录。
...
| Cursor IDE | `.cursor/rules/aegis.mdc`（自动加载） |
| Trae IDE | `.trae/rules/project_rules.md`（自动加载）或 Boot Skill |
| GitHub Copilot | `.github/copilot-instructions.md` |
| Windsurf | `.windsurfrules` |
| 其他 AI 工具 | `AGENTS.md` 通用入口 / Boot Skill 技能导入 |
...
Aegis/
 ...
 skills/
    dev-workflow/                  核心工作流引擎
        SKILL.md
        templates/                 阶段文档模板（9个）
        conventions/               命名和格式约定
        sub-agents/                4 个子代理审查定义
...
📖 人类用户：[QUICK_START.md](docs/QUICK_START.md)（30秒）或 [USER_GUIDE.md](docs/USER_GUIDE.md)（5分钟）
...
| Go | [rules/TechStack/go.md](rules/TechStack/go.md) |
| Rust | [rules/TechStack/rust.md](rules/TechStack/rust.md) |
| Java | [rules/TechStack/java.md](rules/TechStack/java.md) |
| Docker | [rules/TechStack/docker.md](rules/TechStack/docker.md) |
...
v3.0.6  完整介绍见 [Aegis_Intro.md](docs/Aegis_Intro.md)
