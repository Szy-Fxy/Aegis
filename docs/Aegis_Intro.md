...
**输出**：无文档，仅展示改动摘要（文件路径 + 增删行数 + 改动点简述），**不得擅自执行 git/svn 操作**
...
3. 项目根目录 AGENTS.md              ← 项目入口（跨平台 AI 通用入口）
```
...
| 环境 | 入口方式 |
|------|----------|
| Claude Code | `CLAUDE.md`（安装脚本自动生成，含完整行为准则） |
| Cursor IDE | `.cursor/rules/aegis.mdc`（`alwaysApply: true`，自动加载） |
...
│       │   └── sub-agents/              ← 子代理审查定义
│       └── aegis-boot/
│           └── SKILL.md                 ← 启动技能（支持 Skill 自动触发的平台）
│
├── Aegis_Specs/
│   ├── INDEX.md                         ← 需求索引
│   ├── L1/                              ← L1 需求（通常无文档）
...
| 2026-06-04 | v3.1.0 | 流程强化：L1/L2 新增 INDEX.md 登记步骤；L2 新增验收标准用户视角检查；模板新增方案风险边界；全局版本号 3.0.7 → 3.1.0 |
