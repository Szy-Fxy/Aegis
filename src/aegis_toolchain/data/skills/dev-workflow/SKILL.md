...
4. 展示审查结果 + 改动摘要 → 等用户确认
   (**不得擅自执行任何 git/svn 操作**)
5. python -m aegis_toolchain devlog write REQ-xxx -m "改动说明"   ← 自动写 DevLog
6. python -m aegis_toolchain advance                               ← 自动更新 INDEX.md 为 ✅ done
```

**BOUNDARY CHECK**（运行 `python -m aegis_toolchain check`）：
- [ ] INDEX.md 已有当前需求条目？
- [ ] INDEX.md 状态 = ✅ done？
- [ ] DevLog 已写入？
...
| "L1 不需要登记 INDEX.md" | 不。L1 也必须 python -m aegis_toolchain start 登记。 |
| "L1 不需要写 DevLog" | 不。L1 也必须 python -m aegis_toolchain devlog write。 |
...
每个阶段之间，运行 `python -m aegis_toolchain check` 验证。`python -m aegis_toolchain check` 会自动检查当前阶段要求的所有文件是否存在。
...
| "BOUNDARY CHECK 太啰嗦，跳过" | 不。python -m aegis_toolchain check 是一条命令，不啰嗦。 |
| "L2-1 不需要 python -m aegis_toolchain start" | 不。python -m aegis_toolchain start 是入口，不登记=需求不可追踪。 |
...
- **L3-1 开始前**: `python -m aegis_toolchain start "需求名" -l L3`（自动登记到 INDEX.md）
- **每个阶段推进**: AI 创建该阶段的文档 → `python -m aegis_toolchain advance`
- **每次推进前验证**: `python -m aegis_toolchain check`
- **收尾**: `python -m aegis_toolchain devlog write` → `python -m aegis_toolchain advance`（最终 ✅ done）

子代理审查在 L3-6（集成审核）启动，同 L2 的 4 个子代理。

### 迭代退回机制

L3 流程支持在任意阶段退回到上一个阶段。退回后重新创建该阶段文档，然后 `python -m aegis_toolchain advance` 推进。
...
```
□ 0. 文档完整性检查（运行 python -m aegis_toolchain check 验证所有阶段文件）
□ 1. python -m aegis_toolchain devlog write REQ-xxx -m "完整总结"（含背景、改动清单、教训）
